...
def apply_translation(image, binary_mask, tx, ty):
    ...
    translated_person = cv2.warpAffine(
        extracted_person, T_2x3, (w, h), borderMode=cv2.BORDER_CONSTANT, borderValue=(0, 0, 0)
    )
    translated_mask = cv2.warpAffine(
        binary_mask, T_2x3, (w, h), borderMode=cv2.BORDER_CONSTANT, borderValue=0
    )
...
def apply_transformation(image, binary_mask, tx, ty, s, cx, cy):
    ...
    transformed_person = cv2.warpAffine(
        extracted_person, T_2x3, (w, h), borderMode=cv2.BORDER_CONSTANT, borderValue=(0, 0, 0)
    )

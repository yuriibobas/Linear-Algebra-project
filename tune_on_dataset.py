...
def download_dataset():
    ...
    if not os.path.isfile(DATASET_ZIP):
        urllib.request.urlretrieve(DATASET_URL, DATASET_ZIP, reporthook=lambda n, bs, ts: print(f'\r  {min(100, n * bs * 100 // max(ts, 1))}%', end='', flush=True))
        print()
...
def parse_annotation(ann_path):
    ...
    if ann_path.suffix.lower() == '.xml':
        ...
        return boxes
    pattern = re.compile('bounding box for object\\s+\\d+.*?:\\s*\\((\\d+),\\s*(\\d+)\\)\\s*-\\s*\\((\\d+),\\s*(\\d+)\\)', re.IGNORECASE)
...
def run_optuna_search(entries, n_trials=50, iou_threshold=0.5, results_path='dataset_tuning_results_optuna.json'):
    ...
    def objective(trial):
        params = {'win_stride': trial.suggest_categorical('win_stride', ['4,4', '8,8']), 'padding': trial.suggest_categorical('padding', ['8,8', '16,16', '32,32']), 'scale': trial.suggest_float('scale', 1.01, 1.15, step=0.01), 'hit_threshold': trial.suggest_float('hit_threshold', 0.0, 0.8, step=0.1), 'score_threshold': trial.suggest_float('score_threshold', 0.0, 1.0, step=0.1), 'nms_threshold': trial.suggest_float('nms_threshold', 0.1, 0.8, step=0.05), 'use_preprocessing': trial.suggest_categorical('use_preprocessing', [True, False]), 'shrink_factor': trial.suggest_float('shrink_factor', 0.0, 0.25, step=0.02)}
        ...
        for entry in entries:
            ...
            raw_boxes, weights = hog.detectMultiScale(img, winStride=ws, padding=pd, scale=params['scale'], hitThreshold=params['hit_threshold'])
            if len(raw_boxes) > 0:
        ...
        details = {'precision': round(p, 4), 'recall': round(r, 4), 'f1': round(f1, 4), 'mean_iou': round(iou, 4), 'tp': tp, 'fp': fp, 'fn': fn}
        trial.set_user_attr('params', params)
        trial.set_user_attr('details', details)
        log_data = {'trial': trial.number, 'win_stride': list(ws), 'padding': list(pd), 'scale': params['scale'], 'hit_threshold': params['hit_threshold'], 'score_threshold': params['score_threshold'], 'nms_threshold': params['nms_threshold'], 'use_preprocessing': params['use_preprocessing'], 'shrink_factor': round(params['shrink_factor'], 3), **details}
    ...
    for t in pareto_trials:
        pareto_results.append({'trial_id': t.number, 'scores': {'f1': round(t.values[0], 4), 'mean_iou': round(t.values[1], 4)}, 'params': t.user_attrs.get('params', {}), 'details': t.user_attrs.get('details', {})})
    ...
    payload = {'method': 'Multi-Objective Bayesian Optimization (MOTPE)', 'trials': n_trials, 'elapsed_sec': round(elapsed, 1), 'pareto_front_size': len(pareto_trials), 'pareto_front': sorted(pareto_results, key=lambda x: sum(x['scores'].values()), reverse=True), 'all_trials': all_results}

...
def detect_only(image, params):
    return detect_humans(image, win_stride=params['win_stride'], padding=params['padding'], scale=params['scale'], score_threshold=params['score_threshold'], nms_threshold=params['nms_threshold'], hit_threshold=params['hit_threshold'], use_preprocessing=params['use_preprocessing'], shrink_factor=params['shrink_factor'])

def run_pipeline(image, hog_params, grabcut_params):
    pred_boxes = detect_only(image, hog_params)
    if len(pred_boxes) < 2:
        return None
    masks_data = get_binary_masks(image, pred_boxes, top_k=2, grabcut_iters=grabcut_params['grabcut_iters'], morph_kernel_size=grabcut_params['morph_kernel_size'], morph_open_iters=grabcut_params['morph_open_iters'], morph_close_iters=grabcut_params['morph_close_iters'])
    if len(masks_data) < 2:
        return None
    ...
    return {'final': final, 'inpaint_mask': inpaint_mask, 'pred_boxes': pred_boxes, 'masks_data': masks_data}
...
def compute_mask_quality(masks_data):
    ...
    for mask, (x, y, w, h) in masks_data:
        ...
        precision = overlap / mask_area
        recall = overlap / max(box_area, 1)
...
def compute_boundary_continuity(final, inpaint_mask):
    ...
    ring_in = inpaint_mask - eroded
    ring_out = cv2.dilate(inpaint_mask, k3, iterations=1) - inpaint_mask
    if not (np.any(ring_in > 0) and np.any(ring_out > 0)):
        return 0.0
    in_vals = final[ring_in > 0].astype(np.float32)
    out_vals = final[ring_out > 0].astype(np.float32)
...
def _hog_param_suggest(trial):
    raw = {'win_stride': trial.suggest_categorical('win_stride', ['4,4', '8,8']), 'padding': trial.suggest_categorical('padding', ['8,8', '16,16', '32,32']), 'scale': trial.suggest_float('scale', 1.01, 1.1, step=0.01), 'hit_threshold': trial.suggest_float('hit_threshold', 0.0, 0.6, step=0.1), 'score_threshold': trial.suggest_float('score_threshold', 0.0, 0.5, step=0.05), 'nms_threshold': trial.suggest_float('nms_threshold', 0.2, 0.65, step=0.05), 'use_preprocessing': trial.suggest_categorical('use_preprocessing', [True, False]), 'shrink_factor': trial.suggest_float('shrink_factor', 0.0, 0.2, step=0.02)}
...
def tune_phase1(image, gt_boxes, n_trials, n_startup):
    ...
    def objective(trial):
        ...
        all_results.append({'trial': trial.number, 'det_score': round(score, 4), 'n_boxes': len(pred_boxes), **{k: list(v) if isinstance(v, tuple) else v for k, v in params.items()}})
        return score
...
def tune_phase2(image, hog_params, n_trials, n_startup):
    ...
    def objective(trial):
        grabcut_params = {'grabcut_iters': trial.suggest_int('grabcut_iters', 2, 10), 'morph_kernel_size': trial.suggest_categorical('morph_kernel_size', [3, 5, 7]), 'morph_open_iters': trial.suggest_int('morph_open_iters', 1, 3), 'morph_close_iters': trial.suggest_int('morph_close_iters', 1, 3)}
        try:
        ...
        trial.set_user_attr('scores', {'mask_quality': round(mask_quality, 4), 'continuity': round(continuity, 4), 'texture': round(texture, 4)})
        all_results.append({'trial': trial.number, **grabcut_params, 'mask_quality': round(mask_quality, 4), 'continuity': round(continuity, 4), 'texture': round(texture, 4)})
...
def main():
    parser = argparse.ArgumentParser(description='2-Phase Smart Pipeline Tuner (HOG -> GrabCut+Morph).')
    parser.add_argument('--image', default='test_image.jpg')
    parser.add_argument('--gt', default='', help='GT boxes: "x,y,w,h;x,y,w,h"')
    parser.add_argument('--trials1', type=int, default=25, help='Phase 1 (HOG) trials (default: 25, ~30 sec)')
    parser.add_argument('--trials2', type=int, default=35, help='Phase 2 (GrabCut) trials (default: 35, ~5-8 min)')
    parser.add_argument('--phase', choices=['1', '2', 'both'], default='both', help='Which phase to run (default: both)')
    ...
    if args.phase in ('1', 'both'):
        ...
        cfg.read('config.cfg')
        best_hog = {'win_stride': (int(cfg['detection'].get('win_stride_w', 8)), int(cfg['detection'].get('win_stride_h', 8))), 'padding': (int(cfg['detection'].get('padding_w', 8)), int(cfg['detection'].get('padding_h', 8))), 'scale': float(cfg['detection'].get('scale', 1.05)), 'hit_threshold': float(cfg['detection'].get('hit_threshold', 0.0)), 'score_threshold': float(cfg['detection'].get('score_threshold', 0.0)), 'nms_threshold': float(cfg['detection'].get('nms_threshold', 0.25)), 'use_preprocessing': cfg['detection'].get('use_preprocessing', 'true') == 'true', 'shrink_factor': float(cfg['detection'].get('shrink_factor', 0.0))}
        p1_results = []
    ...
    if args.phase in ('2', 'both'):
        ...
        best_grabcut = {'grabcut_iters': 5, 'morph_kernel_size': 5, 'morph_open_iters': 2, 'morph_close_iters': 2}
        p2_results = []
    ...
    payload = {'method': '2-Phase Optuna (HOG -> GrabCut+Morph)', 'total_elapsed_sec': round(total_elapsed, 1), 'best_hog_params': {k: list(v) if isinstance(v, tuple) else v for k, v in best_hog.items()}, 'best_grabcut_params': best_grabcut, 'phase1_trials': p1_results, 'phase2_trials': p2_results}
    ...
    cfg['detection'] = {'win_stride_w': str(ws[0]), 'win_stride_h': str(ws[1]), 'padding_w': str(pd[0]), 'padding_h': str(pd[1]), 'scale': str(best_hog['scale']), 'score_threshold': str(best_hog['score_threshold']), 'nms_threshold': str(best_hog['nms_threshold']), 'hit_threshold': str(best_hog['hit_threshold']), 'use_preprocessing': str(best_hog['use_preprocessing']).lower(), 'shrink_factor': str(best_hog['shrink_factor'])}
    cfg['masks'] = {'grabcut_iters': str(best_grabcut['grabcut_iters']), 'morph_kernel_size': str(best_grabcut['morph_kernel_size']), 'morph_open_iters': str(best_grabcut['morph_open_iters']), 'morph_close_iters': str(best_grabcut['morph_close_iters'])}

...
def tune_full_pipeline(image, gt_boxes=None, max_configs=0, seed=42):
    ...
    for i, values in enumerate(combos):
        ...
        if row["total_score"] > best_score:
            ...
            print(
                f"[{i+1}/{len(combos)}] New best total={best_score:.4f} "
                f"(det={row['det_score']:.4f}, hole={row['hole_score']:.4f}, "
                f"cont={row['continuity_score']:.4f}, tex={row['texture_score']:.4f})"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Tune full detect->mask->swap->inpaint pipeline by visual proxy score."
    )
    ...
    best, top20, tested, valid = tune_full_pipeline(
        image,
        gt_boxes=gt_boxes,

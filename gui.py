...
class GeometricApp:
    ...
    def setup_ui(self):
        ...
        tk.Button(control_frame, text="1. Load Image", command=self.load_image).pack(
            side=tk.LEFT, padx=5, pady=5
        )
        tk.Button(control_frame, text="2. Detect & Extract", command=self.detect).pack(
            side=tk.LEFT, padx=5, pady=5
        )
        tk.Button(control_frame, text="3. Render & Inpaint", command=self.finalize_inpaint).pack(
            side=tk.LEFT, padx=5, pady=5
        )
    ...
    def detect(self):
        ...
        for (mask, rect) in masks_data:
            ...
            cy = rect[1] + rect[3] // 2
            self.sprites.append(
                {"orig_mask": mask, "cx": cx, "cy": cy, "tx": 0, "ty": 0, "s": 1.0}
            )
        ...
        self.status_var.set(
            f"Extracted {len(self.sprites)} people. Drag them or use mousewheel to scale."
        )
        self.render_frame()
    ...
    def render_frame(self):
        ...
        if self.bg_holes is None:
            ...
            for sp in self.sprites:
                trans_img, trans_mask = apply_transformation(
                    self.I_orig,
                    sp["orig_mask"],
                    sp["tx"],
                    sp["ty"],

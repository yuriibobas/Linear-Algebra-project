...
def _nms_xywh(boxes, scores, iou_threshold):
    ...
    while order.size > 0:
        ...
        iou = inter / np.maximum(union, 1e-06)
...
def detect_humans(image, win_stride=(8, 8), padding=(8, 8), scale=1.05, score_threshold=0.0, nms_threshold=0.25, hit_threshold=0.0, use_preprocessing=True, shrink_factor=0.0, backend='opencv'):
    if use_preprocessing:
        image = preprocess_for_hog(image)
    if backend == 'custom':
        boxes, weights = _detect_humans_custom(image=image, win_stride=win_stride, scale=scale, hit_threshold=hit_threshold)
    else:
        hog = _get_opencv_hog_detector()
        boxes, weights = hog.detectMultiScale(image, winStride=win_stride, padding=padding, scale=scale, hitThreshold=hit_threshold)
    if len(boxes) == 0:
        return []
...
def get_binary_masks(image, boxes, top_k=2, grabcut_iters=5, morph_kernel_size=5, morph_open_iters=2, morph_close_iters=2):
    masks = []
    ...
    for x, y, w, h in boxes:
        ...
        binary_mask = cv2.morphologyEx(binary_mask, cv2.MORPH_OPEN, kernel, iterations=morph_open_iters)
        binary_mask = cv2.morphologyEx(binary_mask, cv2.MORPH_CLOSE, kernel, iterations=morph_close_iters)
...
def filter_person_boxes(boxes, image_shape, min_area_ratio=0.01, max_area_ratio=0.65, min_aspect=0.2, max_aspect=1.2):
    if boxes is None:
        return []
    h, w = image_shape[:2]

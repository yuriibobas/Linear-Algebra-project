"""
Automatic parameter tuner for the full detect→mask→swap→inpaint pipeline.

Searches all combinations of HOG/SVM/GrabCut parameters, runs the full
pipeline for each, and scores results using proxy visual-quality metrics.
Saves the best result image and prints the optimal config.cfg values.

Usage:
    python auto_tune.py --image test_image.jpg
    python auto_tune.py --image test_image.jpg --gt "564,110,439,877;46,65,452,903"
"""
...
PARAM_GRID = {
    ...
    "hit_threshold":    [0.0, 0.2, 0.5],
    "score_threshold":  [0.0, 0.1, 0.3],
    "nms_threshold":    [0.25, 0.35, 0.5],
    "use_preprocessing":[True, False],
    "shrink_factor":    [0.0, 0.08, 0.12, 0.15, 0.20],
    "grabcut_iters":    [5, 7, 10],
}
...
def run_pipeline(image, params):
    """Run the full pipeline with given params. Returns (final_image, holes_mask, pred_boxes, masks_data) or None."""
...
def boundary_mae(image, hole_mask):
    """Mean absolute error between hole interior and its 1-pixel boundary ring."""
...
def texture_ratio(image, hole_mask):
    """Ratio of Laplacian variance inside hole vs boundary ring."""
...
def score_result(image, result, gt_boxes):
    """Compute a composite quality score for a pipeline result."""
...
def main():
    parser = argparse.ArgumentParser(
        description="Auto-tune the full pipeline: find best params by exhaustive search."
    )
    ...
    parser.add_argument(
        ...
        help="If >0, randomly sample this many configs instead of full grid.",
    )
    ...
    for i, params in enumerate(combos):
        ...
        if scored["total"] > best_score:
            ...
            print(
                f"  [{i+1}/{len(combos)}] ** NEW BEST total={scored['total']:.4f} "
                f"(det={scored['det_score']:.3f} hole={scored['hole_score']:.3f} "
                f"cont={scored['continuity']:.3f} tex={scored['texture']:.3f})"
            )
            # Print params
            print(
                f"    stride={params['win_stride']} pad={params['padding']} "
                f"scale={params['scale']} hit={params['hit_threshold']} "
                f"score={params['score_threshold']} nms={params['nms_threshold']} "
                f"prep={params['use_preprocessing']} shrink={params['shrink_factor']} "
                f"gc={params['grabcut_iters']}"
            )
    ...
    cfg["detection"] = {
        "win_stride_w": str(ws[0] if isinstance(ws, list) else ws[0]),
        "win_stride_h": str(ws[1] if isinstance(ws, list) else ws[1]),
        "padding_w": str(pd[0] if isinstance(pd, list) else pd[0]),
        "padding_h": str(pd[1] if isinstance(pd, list) else pd[1]),
        "scale": str(best["scale"]),
        "score_threshold": str(best["score_threshold"]),
        "nms_threshold": str(best["nms_threshold"]),
        "hit_threshold": str(best["hit_threshold"]),
        "use_preprocessing": str(best["use_preprocessing"]).lower(),
        "shrink_factor": str(best["shrink_factor"]),
    }

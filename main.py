...
def process_image(
    ...
    # Step 1: Create clean background by inpainting ALL original person locations
    # Dilate masks slightly so the inpainted area fully covers person shadows/edges
    ...
    # Step 2: Composite translated people onto the clean background
    Inew = clean_bg.copy()
    ...
    # Step 3: Feather blending — Gaussian-blur the person edges for seamless compositing
    combined_placed = cv2.bitwise_or(translated_mask_A, translated_mask_B)
    # Create a soft alpha mask: erode to get safe interior, blur the boundary
    ...
    # Where inner mask is 1, alpha is fully 1 (person pixels untouched)
    alpha[inner_mask > 0] = 1.0

    # Blend: person pixels where alpha=1, clean background where alpha=0, smooth transition between
    final_image = (alpha[:, :, None] * Inew.astype(np.float64)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", default="", metavar="FILE",
                        help="Path to a .cfg config file (e.g. config.cfg). "
